...
export default function BasicSetup() {
    const { userData, completeTemplateSetup, updateChosenTemplate, loading, newUserAction, setNewUserAction, chosenTemplate } = useContext(UserContext);
    // const iscompleteTemplateSetup = userData && userData.templateComplete
                            ...
                            <div className='rounded-lg border w-[50%] mx-aut my-12 px-3 py-2 text-sm border-gray-200'>
                                ...
                                <div className='w-[90%] py-4 mx-auto'>
                                    ...
                                    <div onClick={() => setNewUserAction('template')}
                                        className={`border-b flex my-1 cursor-pointer ${completeTemplateSetup ? 'text-primary' : ''} hover:text-primary items-center gap-x-2 py-4 w-[100%] mx-auto`}>

                                        <CgCheck className={`text-lg ${completeTemplateSetup ? 'text-primary' : 'text-gray-500'}`} />
                                        <h1 className='text-sm'>Choose A Template</h1>
                                    </div>


                                    <div onClick={() => setNewUserAction('name')} className='border-b flex my-1 cursor-pointer hover:text-primary items-center gap-x-2 py-4 w-[100%] mx-auto'>
